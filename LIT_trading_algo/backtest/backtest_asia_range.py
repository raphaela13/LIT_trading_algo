...
def backtest_asia_range(symbol, start_date, end_date, rr, buffer, speed_factor):
    # Collection M5 data for the period entered as argument
    m5 = pd.DataFrame(mt.copy_rates_range(symbol, mt.TIMEFRAME_M5, start_date, end_date))
    ...
    stat = bt.run()
    expected_return = ((stat['Win Rate [%]'] / 100) * stat['# Trades'] * rr
                       - (1 - (stat['Win Rate [%]'] / 100)) * stat['# Trades'])

    return [symbol, round(stat['Win Rate [%]'], 3), int(stat['# Trades']), expected_return]